...
SWEP.PrintName			= "PocketWare: First!"
...
SWEP.Sounds["revolver_fire"]	= "weapons/357/357_fire2.wav"

SWEP.Sounds["alyx_first"]		= "pocketware_games/first/alyx_first.wav"
SWEP.Sounds["barney_first"]		= "pocketware_games/first/barney_first.wav"
SWEP.Sounds["breen_first"]		= "pocketware_games/first/breen_first.wav"
SWEP.Sounds["male_first"]		= "pocketware_games/first/male_first.wav"
SWEP.Sounds["vort_first"]		= "pocketware_games/first/vort_first.wav"

SWEP.Sounds["alyx_second"]		= "pocketware_games/first/alyx_second.wav"
SWEP.Sounds["female_second"]	= "pocketware_games/first/female_second.wav"
SWEP.Sounds["male_second"]		= "pocketware_games/first/male_second.wav"

SWEP.Sounds["male_third"]		= "pocketware_games/first/male_third.wav"
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		self:EmitSound( self:GetSound("revolver_fire") )
...
do
	...
	function WARE:OnNPCKilled( victim, killer, weapon )
		if victim:IsNPC() and killer:IsPlayer() then
			if self.ValidKill and (victim.TROLL_GROUP == 1) then
	...
	function WARE:Think( )
		if not self.StartTime then
			...
			self.Call = math.Rand(1.5, 2.7)
			self.ToCall = 1
			self.ValidKill = false
			
		end
		if (self.ToCall <= #self.trolls) and (CurTime() > (self.StartTime + self.Call)) then
			//print( self.spawnedList[self.trolls[self.ToCall].TROLL_GROUP][3] )
			self.trolls[self.ToCall]:EmitSound( GetSound(self.spawnedList[self.trolls[self.ToCall].TROLL_GROUP][3]) )
			self.ValidKill = true
			self.ToCall = self.ToCall + 1
			
			self.Call = self.Call + math.Rand(0.2, 0.6)
...
function SWEP:Deploy()
	if SERVER then	
		...
		local spawnList = {
			{ // 1
				{ "alyx", "models/alyx.mdl", "alyx_first" },
				{ "barney", "models/Barney.mdl", "barney_first" },
				{ "breen", "models/breen.mdl", "breen_first" },
				{ "male", "models/player/Group03/male_07.mdl", "male_first" },
				{ "vort", "models/vortigaunt.mdl", "vort_first" }
			},
			{ // 2
				{ "alyx", "models/alyx.mdl", "alyx_second" },
				{ "male", "models/player/Group03/male_07.mdl", "male_second" },
				{ "female", "models/player/Group03/Female_02.mdl", "female_second" }
			},
			{ // 3
				{ "male", "models/player/Group03/male_07.mdl", "male_third" }
			}
			
			
		}
		
		WARE_TEMP.spawnedList = {}
		local spawnedList = WARE_TEMP.spawnedList
		
		local addition = 1
		local try = 1
		local found = false
		local search = 1
		while (addition <= #spawnList) and (try < 10) do
			local choose = spawnList[addition][ math.random(1, #spawnList[addition] ) ]
			
			found = false
			search = 1
			while (not found) and (search <= #spawnedList) do
				if spawnedList[search][1] == choose[1] then found = true end
				search = search + 1
				
			end
			if not found then
				table.insert( spawnedList, choose )
				try = 1
				addition = addition + 1
		...
		repeat
			tracedata.endpos = tracedata.start + Angle( math.random(10, 15), self.Owner:GetAngles().y + math.random(-50, 50), 0 ):Forward() * 1024
		...
		if canBegin then
			...
			for k,pos in pairs( savedPos ) do
				...
				WareEngine:AppendEntToBin( npc )
				npc.TROLL_GROUP = k
				table.insert( WARE_TEMP.trolls, npc )
				npc:SetModel( spawnedList[k][2] )
				npc:SetPos( pos + Vector(0, 0, 2) )
				npc:SetAngles( Angle(0, (self.Owner:GetPos() - pos):Angle().y, 0) )
				
				npc:Spawn()
				
			end
			WareEngine:HookRules( )
			WareEngine:DrawInstructions( "Kill the person who says First!" )
