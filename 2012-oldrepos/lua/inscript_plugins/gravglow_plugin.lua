PLUGIN.Name = "Gravity Gun Glow"
PLUGIN.DefaultOn = false
PLUGIN.Description = "Adds glow to Gravity Gun."
...
function PLUGIN.HOOK:Think()
	local ent = LocalPlayer():GetDTEntity( 3 )
	
	if (not self.GotEntity) and (ent ~= nil) then
		self.DiscoverTime = CurTime()
		
	end
	
	if (self.GotEntity) and (ent ~= nil) then
		self.LostTime = CurTime()
		
	end
	
	self.GotEntity = ValidEntity(ent)
	if (self.GotEntity) then
		self.GrabEntity = ent
		
		if ((CurTime() - self.LastParticle) > self.ParticleTime) then
			self.LastParticle = CurTime()
			
			local effectData = EffectData()
			effectData:SetEntity( ent )
			util.Effect("effect_mlp_poltergeist", effectData, true, true)
...
function PLUGIN:RenderToStencil( entity )
...
function PLUGIN:RenderToGlowTexture( entity )
	if not ValidEntity( entity ) then return end
	...
	render.SetRenderTarget( RT1 )
		...
		cam.IgnoreZ( true )
			...
			render.SetColorModulation( 1, 0.2, 0.6 )
				SetMaterialOverride( MaterialWhite )
				entity:DrawModel()
				SetMaterialOverride()
...
function PLUGIN.HOOK:PostDrawTranslucentRenderables( )
	if not ValidEntity(self.GrabEntity) then return end
	if (not self.GotEntity and ((CurTime() - self.LostTime) > self.CoolTime)) then return end
	
	if( OUTLINING_ENTITY ) then return end
	OUTLINING_ENTITY = true
	
	self:RenderToStencil( self.GrabEntity )
	self:RenderToGlowTexture( self.GrabEntity )
