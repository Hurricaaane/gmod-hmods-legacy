PLUGIN.Name = "CLASSIC PATTERN MAKER"
	...
	/*table.sort( parts, function( a, b )
		...
		if asDelay and asOn and asChannel and asNote and asVol then
			if sDelay and sOn and sChannel and sNote and sVol then
				return tonumber(asDelay) < tonumber(sDelay)
	...
	for k,line in pairs( parts ) do
		...
		if sDelay and sOn and sChannel and sNote and sVol then
			if sChannel == sChannelToFilter then
				...
				if sOn == "On" and sVol ~= "0" and (first or ((thisTime ~= lastTime) and (NODELAYFILTER and (math.floor(thisTime / DELAYBASE) ~= lastID) or (thisTime % DELAYBASE == 0)))) /*and not channelIsOn*/ then
					--sNoteoffs = sNote
...
function PLUGIN:Load()
	...
	file.Write( "gunstruspmsimple_out.txt", tab .. "\n" .. harmonics )
